...
def run_hedging_simulation(S_path, t_steps, K, T, r, sigma):
    ...
    Rebalances the hedge ratio at every time step (daily).
    """
    dt = T / (len(t_steps) - 1)
    portfolio_values = []
...
def main():
    ...
    print(f"PnL Volatility (Std Dev): ${pnl_std:.2f}")
    print("Note: Variance > 0 indicates hedging slippage (Gamma risk).")
    ...
    ax2.plot(t_grid[:-1], pnl_path, color='#FF6B6B', label='Hedged Portfolio PnL')
    ax2.axhline(0, color='white', linestyle='-', alpha=0.3)
    ax2.set_ylabel('PnL ($)')
    ax2.set_xlabel('Time (Years)')
    ax2.set_title('Delta Hedging Performance')
    ax2.legend()
